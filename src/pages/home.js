import React from "react";
import Image1 from '../images/hero-img.jpg';
import Image2 from '../images/nudail.jpg';
import Image3 from '../images/salad.jpg';
import Image4 from '../images/burger.png';
import Image5 from '../images/sandvich.png';
import Image6 from '../images/ice-cream.png';
import Image7 from '../images/jush.png';
import Image8 from '../images/Fattoush salad.jpg';
import Image9 from '../images/Vegetable salad.jpg';
import Image10 from '../images/Egg salad.jpg';
import Image11 from '../images/Fattoush salad.jpg';
import Image12 from '../images/Fattoush salad.jpg';
import Image13 from '../images/Egg salad.jpg';
import Image14 from '../images/2img.jpg';
import Image15 from '../images/profile-1.jpg';
import Image16 from '../images/profile-2.jpg';
import Image17 from '../images/profile-3.jpg';

function home(){
    return(
        <>
       <section class="py-12 md:py-20 lg:py-24">
      <div class="main-wrapper">
        <div class="flex flex-wrap -mx-4 items-center gap-y-6">
          <div class="xl:w-6/12 lg:w-6/12 md:w-6/12 sm:w-full w-full px-4">
            <h1 class="text-black md:text-[54px] text-[40px] font-extrabold">
              Dive into Delights Of Delectable
              <span class="text-[#39DB4A]">Food</span>
            </h1>
            <p
              class="text-[#4A4A4A] md:text-xl text-lg font-medium mt-[40px] lg:mt-[59px]"
            >
              Where Each Plate Weaves a Story of Culinary Mastery and Passionate
              Craftsmanship
            </p>
            <div
              class="flex items-center mt-[50px] lg:mt-[71px] flex-wrap gap-y-4 lg:flex-nowrap"
            >
              <a
                href="#"
                class="inline-block bg-[#39DB4A] py-3 md:py-4 px-6 md:px-8 rounded-full text-white text-xl lg:text-2xl font-semibold border-2 border-[#39DB4A] hover:bg-white hover:text-[#39DB4A] hover:border-[#39DB4A]"
                >Order Now</a
              >
              <button class="flex items-center ml-4" onclick="toggleModal('modal-id')"
                >Watch Video
                <span
                  class="size-20 bg-white rounded-full flex justify-center items-center shadow-[6px_27px_34px_rgba(0,0,0,0.10)] ml-[29px]"
                  ><svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="28"
                    viewBox="0 0 24 28"
                    fill="none"
                    class="inline-block"
                  >
                    <path
                      d="M22.5 11.4019C24.5 12.5566 24.5 15.4434 22.5 16.5981L4.49999 26.9904C2.49999 28.1451 -1.34201e-06 26.7017 -1.24107e-06 24.3923L-3.32543e-07 3.60769C-2.31596e-07 1.29829 2.5 -0.145083 4.5 1.00962L22.5 11.4019Z"
                      fill="#1E1E1E"
                    />
                  </svg>
                </span>
              </button>
            </div>
          </div>
          <div class="xl:w-6/12 lg:w-6/12 md:w-6/12 sm:w-full w-full px-4">
            <div class="relative">
              <img src={Image1} alt="hero-img" />
              <div
                class="shadow-[26px_22px_51px_rgba(0,0,0,0.14)] absolute top-[20%] md:-left-24 left-0 hidden sm:block"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="248"
                  height="67"
                  viewBox="0 0 248 67"
                  fill="none"
                >
                  <path
                    d="M0 30.1608C0 13.5034 14.1463 0 31.5967 0H213.278C230.728 0 244.875 13.5034 244.875 30.1608C244.875 38.1926 250.305 63.6203 246.849 66.4479C243.394 69.2755 222.314 60.3215 213.278 60.3215H31.5967C14.1463 60.3215 0 46.8181 0 30.1608Z"
                    fill="#FBFBFB"
                  />
                </svg>
                <span
                  class="-mt-[20px] text-[#FF6868] text-base font-semibold inline-block absolute top-10 left-10"
                  >Hot spicy Food 🌶</span
                >
              </div>
              <div
                class="sm:flex gap-x-2 -mt-10 md:-ml-[80px] lg:-ml-0 justify-center hidden "
              >
                <div class="items-center flex pt-[17px] pb-[20px] ps-[12px] gap-[10px] pe-[15px] rounded-[14px] shadow-[7px_35px_51px_0px_rgba(0,0,0,0.14)] bg-[#ffffff]"
                >
                  <img
                    src={Image2}
                    alt=""
                    class="img-fuild rounded-[12px] w-4/12"
                  />
                  <div class="w-8/12">
                    <h4 class="text-[#2C2C2C] text-sm font-semibold">
                      Spicy noodles
                    </h4>
                    <div class="flex gap-x-[5px] my-[5px]">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_263)">
                          <path
                            d="M0.773967 7.23347L2.85063 8.75014L2.06197 11.1926C1.93452 11.5714 1.9329 11.9812 2.05736 12.361C2.18183 12.7408 2.4257 13.0702 2.75263 13.3001C3.07396 13.5374 3.46336 13.6645 3.86281 13.6625C4.26225 13.6605 4.65036 13.5295 4.9693 13.2891L6.99988 11.7946L9.03105 13.2873C9.3518 13.5232 9.7391 13.6514 10.1373 13.6533C10.5354 13.6553 10.924 13.5309 11.247 13.2981C11.57 13.0653 11.8109 12.7361 11.9351 12.3577C12.0592 11.9794 12.0601 11.5715 11.9378 11.1926L11.1491 8.75014L13.2258 7.23347C13.5461 6.99927 13.7843 6.66982 13.9062 6.2922C14.0281 5.91457 14.0275 5.50808 13.9046 5.13078C13.7817 4.75348 13.5427 4.42468 13.2217 4.19133C12.9008 3.95799 12.5143 3.83204 12.1175 3.83147H9.56655L8.79247 1.4188C8.67074 1.03903 8.43154 0.707728 8.10937 0.472674C7.78719 0.237621 7.39869 0.110962 6.99988 0.110962C6.60108 0.110962 6.21258 0.237621 5.8904 0.472674C5.56823 0.707728 5.32903 1.03903 5.2073 1.4188L4.43322 3.83147H1.88463C1.48782 3.83204 1.10133 3.95799 0.780372 4.19133C0.459415 4.42468 0.220407 4.75348 0.0974878 5.13078C-0.0254318 5.50808 -0.0259759 5.91457 0.0959333 6.2922C0.217843 6.66982 0.455969 6.99927 0.7763 7.23347H0.773967Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_263">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_263)">
                          <path
                            d="M0.773967 7.23347L2.85063 8.75014L2.06197 11.1926C1.93452 11.5714 1.9329 11.9812 2.05736 12.361C2.18183 12.7408 2.4257 13.0702 2.75263 13.3001C3.07396 13.5374 3.46336 13.6645 3.86281 13.6625C4.26225 13.6605 4.65036 13.5295 4.9693 13.2891L6.99988 11.7946L9.03105 13.2873C9.3518 13.5232 9.7391 13.6514 10.1373 13.6533C10.5354 13.6553 10.924 13.5309 11.247 13.2981C11.57 13.0653 11.8109 12.7361 11.9351 12.3577C12.0592 11.9794 12.0601 11.5715 11.9378 11.1926L11.1491 8.75014L13.2258 7.23347C13.5461 6.99927 13.7843 6.66982 13.9062 6.2922C14.0281 5.91457 14.0275 5.50808 13.9046 5.13078C13.7817 4.75348 13.5427 4.42468 13.2217 4.19133C12.9008 3.95799 12.5143 3.83204 12.1175 3.83147H9.56655L8.79247 1.4188C8.67074 1.03903 8.43154 0.707728 8.10937 0.472674C7.78719 0.237621 7.39869 0.110962 6.99988 0.110962C6.60108 0.110962 6.21258 0.237621 5.8904 0.472674C5.56823 0.707728 5.32903 1.03903 5.2073 1.4188L4.43322 3.83147H1.88463C1.48782 3.83204 1.10133 3.95799 0.780372 4.19133C0.459415 4.42468 0.220407 4.75348 0.0974878 5.13078C-0.0254318 5.50808 -0.0259759 5.91457 0.0959333 6.2922C0.217843 6.66982 0.455969 6.99927 0.7763 7.23347H0.773967Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_263">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_263)">
                          <path
                            d="M0.773967 7.23347L2.85063 8.75014L2.06197 11.1926C1.93452 11.5714 1.9329 11.9812 2.05736 12.361C2.18183 12.7408 2.4257 13.0702 2.75263 13.3001C3.07396 13.5374 3.46336 13.6645 3.86281 13.6625C4.26225 13.6605 4.65036 13.5295 4.9693 13.2891L6.99988 11.7946L9.03105 13.2873C9.3518 13.5232 9.7391 13.6514 10.1373 13.6533C10.5354 13.6553 10.924 13.5309 11.247 13.2981C11.57 13.0653 11.8109 12.7361 11.9351 12.3577C12.0592 11.9794 12.0601 11.5715 11.9378 11.1926L11.1491 8.75014L13.2258 7.23347C13.5461 6.99927 13.7843 6.66982 13.9062 6.2922C14.0281 5.91457 14.0275 5.50808 13.9046 5.13078C13.7817 4.75348 13.5427 4.42468 13.2217 4.19133C12.9008 3.95799 12.5143 3.83204 12.1175 3.83147H9.56655L8.79247 1.4188C8.67074 1.03903 8.43154 0.707728 8.10937 0.472674C7.78719 0.237621 7.39869 0.110962 6.99988 0.110962C6.60108 0.110962 6.21258 0.237621 5.8904 0.472674C5.56823 0.707728 5.32903 1.03903 5.2073 1.4188L4.43322 3.83147H1.88463C1.48782 3.83204 1.10133 3.95799 0.780372 4.19133C0.459415 4.42468 0.220407 4.75348 0.0974878 5.13078C-0.0254318 5.50808 -0.0259759 5.91457 0.0959333 6.2922C0.217843 6.66982 0.455969 6.99927 0.7763 7.23347H0.773967Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_263">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_266)">
                          <path
                            d="M3.99702 13.6718C3.55182 13.6698 3.11869 13.5269 2.75977 13.2635C2.39297 13.0054 2.11927 12.6359 1.97938 12.2098C1.83949 11.7837 1.84088 11.3239 1.98335 10.8986L2.66585 8.78754L0.869185 7.47387C0.508943 7.21067 0.241179 6.84027 0.104203 6.41567C-0.0327735 5.99107 -0.0319429 5.53403 0.106576 5.10993C0.245094 4.68583 0.514203 4.3164 0.875399 4.05452C1.2366 3.79263 1.67137 3.65169 2.11752 3.65187H4.31668L4.9846 1.56937C5.12168 1.14292 5.39057 0.770975 5.75255 0.50711C6.11452 0.243245 6.55091 0.101074 6.99885 0.101074C7.44679 0.101074 7.88318 0.243245 8.24516 0.50711C8.60713 0.770975 8.87602 1.14292 9.0131 1.56937V1.56937L9.68335 3.65187H11.8825C12.3286 3.65181 12.7633 3.79284 13.1244 4.05477C13.4855 4.31671 13.7545 4.68613 13.893 5.1102C14.0314 5.53428 14.0322 5.99127 13.8952 6.41581C13.7582 6.84036 13.4905 7.2107 13.1303 7.47387L11.3342 8.78754L12.0167 10.8986C12.1543 11.3245 12.1533 11.783 12.0138 12.2082C11.8743 12.6334 11.6036 13.0035 11.2405 13.2651C10.8774 13.5267 10.4407 13.6664 9.99319 13.6641C9.54567 13.6619 9.11041 13.5177 8.75002 13.2524L7.00002 11.9655L5.25002 13.253C4.88792 13.523 4.44874 13.6698 3.99702 13.6718V13.6718ZM2.11752 5.40187C2.04052 5.40186 1.96549 5.42619 1.90315 5.47139C1.84081 5.51658 1.79435 5.58032 1.77041 5.6535C1.74647 5.72668 1.74628 5.80555 1.76985 5.87885C1.79343 5.95215 1.83957 6.01612 1.90168 6.06162L4.20935 7.74804C4.35795 7.85669 4.46849 8.00944 4.52525 8.18455C4.58202 8.35966 4.58211 8.54821 4.52552 8.72337L3.65052 11.4359C3.62624 11.5094 3.62599 11.5888 3.64981 11.6626C3.67363 11.7363 3.72028 11.8005 3.78302 11.846C3.84576 11.8915 3.92133 11.9158 3.99881 11.9155C4.07629 11.9151 4.15166 11.8902 4.21402 11.8442L6.4826 10.1753C6.63289 10.0647 6.81459 10.0051 7.00118 10.0051C7.18778 10.0051 7.36948 10.0647 7.51977 10.1753L9.78835 11.8436C9.85063 11.89 9.9261 11.9153 10.0038 11.9159C10.0814 11.9164 10.1572 11.8921 10.2201 11.8465C10.283 11.8009 10.3297 11.7365 10.3534 11.6625C10.3771 11.5885 10.3766 11.5089 10.3519 11.4353L9.47685 8.72279C9.42026 8.54762 9.42035 8.35907 9.47711 8.18396C9.53388 8.00885 9.64442 7.85611 9.79302 7.74746L12.1001 6.06104C12.1622 6.01557 12.2083 5.95165 12.2319 5.87841C12.2555 5.80517 12.2553 5.72634 12.2315 5.65319C12.2076 5.58004 12.1612 5.5163 12.099 5.47106C12.0367 5.42582 11.9618 5.4014 11.8849 5.40129H9.04168C8.85636 5.40124 8.67584 5.34236 8.52613 5.23312C8.37643 5.12388 8.26527 4.96993 8.20868 4.79346L7.35002 2.10371C7.32637 2.03009 7.27996 1.96588 7.21748 1.92033C7.155 1.87477 7.07967 1.85023 7.00235 1.85023C6.92503 1.85023 6.8497 1.87477 6.78722 1.92033C6.72474 1.96588 6.67833 2.03009 6.65468 2.10371L5.7896 4.79404C5.73311 4.97023 5.62222 5.12397 5.47286 5.23319C5.32351 5.3424 5.14338 5.40146 4.95835 5.40187H2.11752Z"
                            fill="#B0B4B7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_266">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_266)">
                          <path
                            d="M3.99702 13.6718C3.55182 13.6698 3.11869 13.5269 2.75977 13.2635C2.39297 13.0054 2.11927 12.6359 1.97938 12.2098C1.83949 11.7837 1.84088 11.3239 1.98335 10.8986L2.66585 8.78754L0.869185 7.47387C0.508943 7.21067 0.241179 6.84027 0.104203 6.41567C-0.0327735 5.99107 -0.0319429 5.53403 0.106576 5.10993C0.245094 4.68583 0.514203 4.3164 0.875399 4.05452C1.2366 3.79263 1.67137 3.65169 2.11752 3.65187H4.31668L4.9846 1.56937C5.12168 1.14292 5.39057 0.770975 5.75255 0.50711C6.11452 0.243245 6.55091 0.101074 6.99885 0.101074C7.44679 0.101074 7.88318 0.243245 8.24516 0.50711C8.60713 0.770975 8.87602 1.14292 9.0131 1.56937V1.56937L9.68335 3.65187H11.8825C12.3286 3.65181 12.7633 3.79284 13.1244 4.05477C13.4855 4.31671 13.7545 4.68613 13.893 5.1102C14.0314 5.53428 14.0322 5.99127 13.8952 6.41581C13.7582 6.84036 13.4905 7.2107 13.1303 7.47387L11.3342 8.78754L12.0167 10.8986C12.1543 11.3245 12.1533 11.783 12.0138 12.2082C11.8743 12.6334 11.6036 13.0035 11.2405 13.2651C10.8774 13.5267 10.4407 13.6664 9.99319 13.6641C9.54567 13.6619 9.11041 13.5177 8.75002 13.2524L7.00002 11.9655L5.25002 13.253C4.88792 13.523 4.44874 13.6698 3.99702 13.6718V13.6718ZM2.11752 5.40187C2.04052 5.40186 1.96549 5.42619 1.90315 5.47139C1.84081 5.51658 1.79435 5.58032 1.77041 5.6535C1.74647 5.72668 1.74628 5.80555 1.76985 5.87885C1.79343 5.95215 1.83957 6.01612 1.90168 6.06162L4.20935 7.74804C4.35795 7.85669 4.46849 8.00944 4.52525 8.18455C4.58202 8.35966 4.58211 8.54821 4.52552 8.72337L3.65052 11.4359C3.62624 11.5094 3.62599 11.5888 3.64981 11.6626C3.67363 11.7363 3.72028 11.8005 3.78302 11.846C3.84576 11.8915 3.92133 11.9158 3.99881 11.9155C4.07629 11.9151 4.15166 11.8902 4.21402 11.8442L6.4826 10.1753C6.63289 10.0647 6.81459 10.0051 7.00118 10.0051C7.18778 10.0051 7.36948 10.0647 7.51977 10.1753L9.78835 11.8436C9.85063 11.89 9.9261 11.9153 10.0038 11.9159C10.0814 11.9164 10.1572 11.8921 10.2201 11.8465C10.283 11.8009 10.3297 11.7365 10.3534 11.6625C10.3771 11.5885 10.3766 11.5089 10.3519 11.4353L9.47685 8.72279C9.42026 8.54762 9.42035 8.35907 9.47711 8.18396C9.53388 8.00885 9.64442 7.85611 9.79302 7.74746L12.1001 6.06104C12.1622 6.01557 12.2083 5.95165 12.2319 5.87841C12.2555 5.80517 12.2553 5.72634 12.2315 5.65319C12.2076 5.58004 12.1612 5.5163 12.099 5.47106C12.0367 5.42582 11.9618 5.4014 11.8849 5.40129H9.04168C8.85636 5.40124 8.67584 5.34236 8.52613 5.23312C8.37643 5.12388 8.26527 4.96993 8.20868 4.79346L7.35002 2.10371C7.32637 2.03009 7.27996 1.96588 7.21748 1.92033C7.155 1.87477 7.07967 1.85023 7.00235 1.85023C6.92503 1.85023 6.8497 1.87477 6.78722 1.92033C6.72474 1.96588 6.67833 2.03009 6.65468 2.10371L5.7896 4.79404C5.73311 4.97023 5.62222 5.12397 5.47286 5.23319C5.32351 5.3424 5.14338 5.40146 4.95835 5.40187H2.11752Z"
                            fill="#B0B4B7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_266">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-sm text-[#515151] font-semibold">
                      <span class="text-[#FF7979] text-lg">$</span>18.00
                    </p>
                  </div>
                </div>
                <div class="items-center flex pt-[17px] pb-[20px] ps-[12px] gap-[10px] pe-[15px] rounded-[14px] shadow-[7px_35px_51px_0px_rgba(0,0,0,0.14)]  bg-[#ffffff]"
                >
                  <img
                    src={Image3}
                    alt=""
                    class="img-fuild rounded-[12px] w-4/12"
                  />
                  <div class="w-8/12">
                    <h6 class="text-[#2C2C2C] text-sm font-semibold">
                      Vegetarian salad
                    </h6>
                    <div class="flex gap-x-[5px] my-[5px]">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_263)">
                          <path
                            d="M0.773967 7.23347L2.85063 8.75014L2.06197 11.1926C1.93452 11.5714 1.9329 11.9812 2.05736 12.361C2.18183 12.7408 2.4257 13.0702 2.75263 13.3001C3.07396 13.5374 3.46336 13.6645 3.86281 13.6625C4.26225 13.6605 4.65036 13.5295 4.9693 13.2891L6.99988 11.7946L9.03105 13.2873C9.3518 13.5232 9.7391 13.6514 10.1373 13.6533C10.5354 13.6553 10.924 13.5309 11.247 13.2981C11.57 13.0653 11.8109 12.7361 11.9351 12.3577C12.0592 11.9794 12.0601 11.5715 11.9378 11.1926L11.1491 8.75014L13.2258 7.23347C13.5461 6.99927 13.7843 6.66982 13.9062 6.2922C14.0281 5.91457 14.0275 5.50808 13.9046 5.13078C13.7817 4.75348 13.5427 4.42468 13.2217 4.19133C12.9008 3.95799 12.5143 3.83204 12.1175 3.83147H9.56655L8.79247 1.4188C8.67074 1.03903 8.43154 0.707728 8.10937 0.472674C7.78719 0.237621 7.39869 0.110962 6.99988 0.110962C6.60108 0.110962 6.21258 0.237621 5.8904 0.472674C5.56823 0.707728 5.32903 1.03903 5.2073 1.4188L4.43322 3.83147H1.88463C1.48782 3.83204 1.10133 3.95799 0.780372 4.19133C0.459415 4.42468 0.220407 4.75348 0.0974878 5.13078C-0.0254318 5.50808 -0.0259759 5.91457 0.0959333 6.2922C0.217843 6.66982 0.455969 6.99927 0.7763 7.23347H0.773967Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_263">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_263)">
                          <path
                            d="M0.773967 7.23347L2.85063 8.75014L2.06197 11.1926C1.93452 11.5714 1.9329 11.9812 2.05736 12.361C2.18183 12.7408 2.4257 13.0702 2.75263 13.3001C3.07396 13.5374 3.46336 13.6645 3.86281 13.6625C4.26225 13.6605 4.65036 13.5295 4.9693 13.2891L6.99988 11.7946L9.03105 13.2873C9.3518 13.5232 9.7391 13.6514 10.1373 13.6533C10.5354 13.6553 10.924 13.5309 11.247 13.2981C11.57 13.0653 11.8109 12.7361 11.9351 12.3577C12.0592 11.9794 12.0601 11.5715 11.9378 11.1926L11.1491 8.75014L13.2258 7.23347C13.5461 6.99927 13.7843 6.66982 13.9062 6.2922C14.0281 5.91457 14.0275 5.50808 13.9046 5.13078C13.7817 4.75348 13.5427 4.42468 13.2217 4.19133C12.9008 3.95799 12.5143 3.83204 12.1175 3.83147H9.56655L8.79247 1.4188C8.67074 1.03903 8.43154 0.707728 8.10937 0.472674C7.78719 0.237621 7.39869 0.110962 6.99988 0.110962C6.60108 0.110962 6.21258 0.237621 5.8904 0.472674C5.56823 0.707728 5.32903 1.03903 5.2073 1.4188L4.43322 3.83147H1.88463C1.48782 3.83204 1.10133 3.95799 0.780372 4.19133C0.459415 4.42468 0.220407 4.75348 0.0974878 5.13078C-0.0254318 5.50808 -0.0259759 5.91457 0.0959333 6.2922C0.217843 6.66982 0.455969 6.99927 0.7763 7.23347H0.773967Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_263">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_263)">
                          <path
                            d="M0.773967 7.23347L2.85063 8.75014L2.06197 11.1926C1.93452 11.5714 1.9329 11.9812 2.05736 12.361C2.18183 12.7408 2.4257 13.0702 2.75263 13.3001C3.07396 13.5374 3.46336 13.6645 3.86281 13.6625C4.26225 13.6605 4.65036 13.5295 4.9693 13.2891L6.99988 11.7946L9.03105 13.2873C9.3518 13.5232 9.7391 13.6514 10.1373 13.6533C10.5354 13.6553 10.924 13.5309 11.247 13.2981C11.57 13.0653 11.8109 12.7361 11.9351 12.3577C12.0592 11.9794 12.0601 11.5715 11.9378 11.1926L11.1491 8.75014L13.2258 7.23347C13.5461 6.99927 13.7843 6.66982 13.9062 6.2922C14.0281 5.91457 14.0275 5.50808 13.9046 5.13078C13.7817 4.75348 13.5427 4.42468 13.2217 4.19133C12.9008 3.95799 12.5143 3.83204 12.1175 3.83147H9.56655L8.79247 1.4188C8.67074 1.03903 8.43154 0.707728 8.10937 0.472674C7.78719 0.237621 7.39869 0.110962 6.99988 0.110962C6.60108 0.110962 6.21258 0.237621 5.8904 0.472674C5.56823 0.707728 5.32903 1.03903 5.2073 1.4188L4.43322 3.83147H1.88463C1.48782 3.83204 1.10133 3.95799 0.780372 4.19133C0.459415 4.42468 0.220407 4.75348 0.0974878 5.13078C-0.0254318 5.50808 -0.0259759 5.91457 0.0959333 6.2922C0.217843 6.66982 0.455969 6.99927 0.7763 7.23347H0.773967Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_263">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_263)">
                          <path
                            d="M0.773967 7.23347L2.85063 8.75014L2.06197 11.1926C1.93452 11.5714 1.9329 11.9812 2.05736 12.361C2.18183 12.7408 2.4257 13.0702 2.75263 13.3001C3.07396 13.5374 3.46336 13.6645 3.86281 13.6625C4.26225 13.6605 4.65036 13.5295 4.9693 13.2891L6.99988 11.7946L9.03105 13.2873C9.3518 13.5232 9.7391 13.6514 10.1373 13.6533C10.5354 13.6553 10.924 13.5309 11.247 13.2981C11.57 13.0653 11.8109 12.7361 11.9351 12.3577C12.0592 11.9794 12.0601 11.5715 11.9378 11.1926L11.1491 8.75014L13.2258 7.23347C13.5461 6.99927 13.7843 6.66982 13.9062 6.2922C14.0281 5.91457 14.0275 5.50808 13.9046 5.13078C13.7817 4.75348 13.5427 4.42468 13.2217 4.19133C12.9008 3.95799 12.5143 3.83204 12.1175 3.83147H9.56655L8.79247 1.4188C8.67074 1.03903 8.43154 0.707728 8.10937 0.472674C7.78719 0.237621 7.39869 0.110962 6.99988 0.110962C6.60108 0.110962 6.21258 0.237621 5.8904 0.472674C5.56823 0.707728 5.32903 1.03903 5.2073 1.4188L4.43322 3.83147H1.88463C1.48782 3.83204 1.10133 3.95799 0.780372 4.19133C0.459415 4.42468 0.220407 4.75348 0.0974878 5.13078C-0.0254318 5.50808 -0.0259759 5.91457 0.0959333 6.2922C0.217843 6.66982 0.455969 6.99927 0.7763 7.23347H0.773967Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_263">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_266)">
                          <path
                            d="M3.99702 13.6718C3.55182 13.6698 3.11869 13.5269 2.75977 13.2635C2.39297 13.0054 2.11927 12.6359 1.97938 12.2098C1.83949 11.7837 1.84088 11.3239 1.98335 10.8986L2.66585 8.78754L0.869185 7.47387C0.508943 7.21067 0.241179 6.84027 0.104203 6.41567C-0.0327735 5.99107 -0.0319429 5.53403 0.106576 5.10993C0.245094 4.68583 0.514203 4.3164 0.875399 4.05452C1.2366 3.79263 1.67137 3.65169 2.11752 3.65187H4.31668L4.9846 1.56937C5.12168 1.14292 5.39057 0.770975 5.75255 0.50711C6.11452 0.243245 6.55091 0.101074 6.99885 0.101074C7.44679 0.101074 7.88318 0.243245 8.24516 0.50711C8.60713 0.770975 8.87602 1.14292 9.0131 1.56937V1.56937L9.68335 3.65187H11.8825C12.3286 3.65181 12.7633 3.79284 13.1244 4.05477C13.4855 4.31671 13.7545 4.68613 13.893 5.1102C14.0314 5.53428 14.0322 5.99127 13.8952 6.41581C13.7582 6.84036 13.4905 7.2107 13.1303 7.47387L11.3342 8.78754L12.0167 10.8986C12.1543 11.3245 12.1533 11.783 12.0138 12.2082C11.8743 12.6334 11.6036 13.0035 11.2405 13.2651C10.8774 13.5267 10.4407 13.6664 9.99319 13.6641C9.54567 13.6619 9.11041 13.5177 8.75002 13.2524L7.00002 11.9655L5.25002 13.253C4.88792 13.523 4.44874 13.6698 3.99702 13.6718V13.6718ZM2.11752 5.40187C2.04052 5.40186 1.96549 5.42619 1.90315 5.47139C1.84081 5.51658 1.79435 5.58032 1.77041 5.6535C1.74647 5.72668 1.74628 5.80555 1.76985 5.87885C1.79343 5.95215 1.83957 6.01612 1.90168 6.06162L4.20935 7.74804C4.35795 7.85669 4.46849 8.00944 4.52525 8.18455C4.58202 8.35966 4.58211 8.54821 4.52552 8.72337L3.65052 11.4359C3.62624 11.5094 3.62599 11.5888 3.64981 11.6626C3.67363 11.7363 3.72028 11.8005 3.78302 11.846C3.84576 11.8915 3.92133 11.9158 3.99881 11.9155C4.07629 11.9151 4.15166 11.8902 4.21402 11.8442L6.4826 10.1753C6.63289 10.0647 6.81459 10.0051 7.00118 10.0051C7.18778 10.0051 7.36948 10.0647 7.51977 10.1753L9.78835 11.8436C9.85063 11.89 9.9261 11.9153 10.0038 11.9159C10.0814 11.9164 10.1572 11.8921 10.2201 11.8465C10.283 11.8009 10.3297 11.7365 10.3534 11.6625C10.3771 11.5885 10.3766 11.5089 10.3519 11.4353L9.47685 8.72279C9.42026 8.54762 9.42035 8.35907 9.47711 8.18396C9.53388 8.00885 9.64442 7.85611 9.79302 7.74746L12.1001 6.06104C12.1622 6.01557 12.2083 5.95165 12.2319 5.87841C12.2555 5.80517 12.2553 5.72634 12.2315 5.65319C12.2076 5.58004 12.1612 5.5163 12.099 5.47106C12.0367 5.42582 11.9618 5.4014 11.8849 5.40129H9.04168C8.85636 5.40124 8.67584 5.34236 8.52613 5.23312C8.37643 5.12388 8.26527 4.96993 8.20868 4.79346L7.35002 2.10371C7.32637 2.03009 7.27996 1.96588 7.21748 1.92033C7.155 1.87477 7.07967 1.85023 7.00235 1.85023C6.92503 1.85023 6.8497 1.87477 6.78722 1.92033C6.72474 1.96588 6.67833 2.03009 6.65468 2.10371L5.7896 4.79404C5.73311 4.97023 5.62222 5.12397 5.47286 5.23319C5.32351 5.3424 5.14338 5.40146 4.95835 5.40187H2.11752Z"
                            fill="#B0B4B7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_266">
                            <rect width="14" height="14" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-sm text-[#515151] font-semibold">
                      <span class="text-[#FF7979] text-lg">$</span>23.00
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="py-12 md:py-20 lg:py-24">
      <div class="main-wrapper">
        <div class="text-center mb-[78px]">
          <p
            class="text-[#FF6868] md:text-xl text-lg font-bold uppercase tracking-[3.5px]"
          >
            Customer Favorites
          </p>
          <h2 class="text-black md:text-5xl text-[32px] font-bold mt-7">
            Popular Catagories
          </h2>
        </div>
        <div class="flex flex-wrap -mx-4 gap-y-6 gap-y-5">
          <div class="xl:w-3/12 lg:w-3/12 md:w-6/12 sm:w-6/12 w-full px-4">
            <div
              class="pt-[39px] pb-[46px] px-5 bg-white rounded-[40px] shadow-[2px_10px_30px_0px_rgba(0,0,0,0.10)] text-center hover:scale-110"
            >
              <div
                class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
              >
                <img src={Image4} alt="burger" />
              </div>
              <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                Main Dish
              </h4>
              <p class="text-[#555555] font-medium md:text-xl text-lg mt-3">
                (86 dishes)
              </p>
            </div>
          </div>
          <div class="xl:w-3/12 lg:w-3/12 md:w-6/12 sm:w-6/12 w-full px-4">
            <div
              class="pt-[39px] pb-[46px] px-5 bg-white rounded-[40px] shadow-[2px_10px_30px_0px_rgba(0,0,0,0.10)] text-center hover:scale-110"
            >
              <div
                class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
              >
                <img src={Image5} alt="burger" />
              </div>
              <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                Break Fast
              </h4>
              <p class="text-[#555555] font-medium md:text-xl text-lg mt-3">
                (12 break fast)
              </p>
            </div>
          </div>
          <div class="xl:w-3/12 lg:w-3/12 md:w-6/12 sm:w-6/12 w-full px-4">
            <div
              class="pt-[39px] pb-[46px] px-5 bg-white rounded-[40px] shadow-[2px_10px_30px_0px_rgba(0,0,0,0.10)] text-center hover:scale-110"
            >
              <div
                class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
              >
                <img src={Image6} alt="burger" />
              </div>
              <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                Dessert
              </h4>
              <p class="text-[#555555] font-medium md:text-xl text-lg mt-3">
                (48 dessert)
              </p>
            </div>
          </div>
          <div class="xl:w-3/12 lg:w-3/12 md:w-6/12 sm:w-6/12 w-full px-4">
            <div
              class="pt-[39px] pb-[46px] px-5 bg-white rounded-[40px] shadow-[2px_10px_30px_0px_rgba(0,0,0,0.10)] text-center hover:scale-110"
            >
              <div
                class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
              >
                <img src={Image7} alt="burger" />
              </div>
              <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                Browse All
              </h4>
              <p class="text-[#555555] font-medium md:text-xl text-lg mt-3">
                (255 Items)
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="py-12 md:py-20 lg:py-24">
      <div class="main-wrapper">
        <div
          class=" mb-[40px] md:mb-[80px] flex justify-between flex-wrap md:flex-nowrap gap-y-6"
        >
          <div>
            <p
              class="text-[#FF6868] md:text-xl text-lg font-bold uppercase tracking-[3.5px]"
            >
              Special Dishes
            </p>
            <h2
              class="text-black md:text-5xl text-[32px] font-bold mt-7 md:w-6/12"
            >
              Standout Dishes From Our Menu
            </h2>
          </div>
          <div class="flex gap-x-12 items-end ml-auto mr-0">
            <div
              class="swiper-button-prev bg-[#39DB4A] size-20 rounded-full text-white static"
            ></div>
            <div
              class="swiper-button-next bg-[#39DB4A] size-20 rounded-full text-white static"
            ></div>
          </div>
        </div>

        <div class="swiper mySwiper">
          <div class="swiper-wrapper py-6">
            <div class="swiper-slide">
              <div
                class="pt-[39px] pb-[46px] px-[40px] bg-white rounded-[40px] shadow-[2px_9px_42px_0px_rgba(0,0,0,0.08)] relative"
              >
                <div
                  class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
                >
                  <img src={Image8} alt="burger" />
                </div>
                <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                  Fattoush salad
                </h4>
                <p class="text-[#555555] font-medium text-[22px] mt-3">
                  Description of the item
                </p>
                <div class="flex items-center justify-between mt-2.5">
                  <p class="text-2xl font-semibold text-[#515151]">
                    <span class="text-xl text-[#FF7979]">$</span>24.00
                  </p>
                  <div class="flex items-center">
                    <div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_97)">
                          <path
                            d="M1.3268 12.4003L4.8868 15.0003L3.5348 19.1873C3.31631 19.8367 3.31354 20.5393 3.52691 21.1904C3.74027 21.8415 4.15834 22.4062 4.7188 22.8003C5.26965 23.2071 5.93719 23.425 6.62195 23.4216C7.30671 23.4182 7.97204 23.1936 8.5188 22.7813L11.9998 20.2193L15.4818 22.7783C16.0317 23.1828 16.6956 23.4024 17.3782 23.4058C18.0607 23.4091 18.7268 23.1959 19.2806 22.7968C19.8343 22.3977 20.2473 21.8333 20.4601 21.1848C20.6729 20.5362 20.6745 19.8369 20.4648 19.1873L19.1128 15.0003L22.6728 12.4003C23.2219 11.9988 23.6302 11.4341 23.8391 10.7867C24.0481 10.1393 24.0472 9.44249 23.8365 8.79569C23.6258 8.1489 23.216 7.58523 22.6658 7.18521C22.1156 6.78519 21.4531 6.56927 20.7728 6.56831H16.3998L15.0728 2.43231C14.8641 1.78126 14.4541 1.21332 13.9018 0.810371C13.3495 0.407422 12.6835 0.190292 11.9998 0.190292C11.3161 0.190292 10.6501 0.407422 10.0978 0.810371C9.54553 1.21332 9.13548 1.78126 8.9268 2.43231L7.5998 6.56831H3.2308C2.55054 6.56927 1.88799 6.78519 1.33778 7.18521C0.787564 7.58523 0.377837 8.1489 0.167118 8.79569C-0.0436018 9.44249 -0.0445344 10.1393 0.164453 10.7867C0.37344 11.4341 0.781657 11.9988 1.3308 12.4003H1.3268Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_97">
                            <rect width="24" height="24" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-[#454545] text-2xl font-semibold ml-[15px]">
                      4.9
                    </p>
                  </div>
                </div>
                <div
                  class="absolute top-0 right-0 rounded-tr-[37.5px] rounded-bl-[42.5px] bg-[#39DB4A] px-6 py-8"
                >
                  <span class="inline-block"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <g clip-path="url(#clip0_1_105)">
                        <path
                          d="M17.5 1.91653C16.3739 1.93405 15.2724 2.24839 14.3068 2.82781C13.3411 3.40722 12.5453 4.2312 12 5.21653C11.4546 4.2312 10.6589 3.40722 9.69323 2.82781C8.72753 2.24839 7.62604 1.93405 6.49999 1.91653C4.70493 1.99453 3.01369 2.77979 1.79577 4.10077C0.577848 5.42175 -0.0677617 7.17106 -1.11917e-05 8.96653C-1.11917e-05 13.5135 4.78599 18.4795 8.79999 21.8465C9.69621 22.5997 10.8293 23.0126 12 23.0126C13.1706 23.0126 14.3038 22.5997 15.2 21.8465C19.214 18.4795 24 13.5135 24 8.96653C24.0677 7.17106 23.4221 5.42175 22.2042 4.10077C20.9863 2.77979 19.295 1.99453 17.5 1.91653Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1_105">
                          <rect width="24" height="24" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </span>
                </div>
              </div>
            </div>
            <div class="swiper-slide">
              <div
                class="pt-[39px] pb-[46px] px-[40px] bg-white rounded-[40px] shadow-[2px_9px_42px_0px_rgba(0,0,0,0.08)] relative"
              >
                <div
                  class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
                >
                  <img src={Image9} alt="burger" />
                </div>
                <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                  Vegetable salad
                </h4>
                <p class="text-[#555555] font-medium text-[22px] mt-3">
                  Description of the item
                </p>
                <div class="flex items-center justify-between mt-2.5">
                  <p class="text-2xl font-semibold text-[#515151]">
                    <span class="text-xl text-[#FF7979]">$</span>26.00
                  </p>
                  <div class="flex items-center">
                    <div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_103)">
                          <path
                            d="M17.25 1.85071C16.2243 1.86063 15.2152 2.11065 14.3035 2.58073C13.3918 3.05081 12.6029 3.72788 12 4.55771C11.397 3.72788 10.6081 3.05081 9.69644 2.58073C8.78476 2.11065 7.77565 1.86063 6.74996 1.85071C4.89173 1.92491 3.13848 2.73189 1.87358 4.09517C0.608672 5.45846 -0.0649657 7.26713 -4.03235e-05 9.12571C-4.03235e-05 13.6777 4.67396 18.5507 8.59996 21.8377C9.55329 22.6393 10.7589 23.0788 12.0045 23.0788C13.25 23.0788 14.4556 22.6393 15.409 21.8377C19.331 18.5507 24.009 13.6777 24.009 9.12571C24.0738 7.26563 23.399 5.45564 22.1322 4.0921C20.8653 2.72856 19.1098 1.9226 17.25 1.85071ZM13.477 19.5387C13.0634 19.8869 12.5401 20.0779 11.9995 20.0779C11.4588 20.0779 10.9355 19.8869 10.522 19.5387C5.74196 15.5307 2.99996 11.7357 2.99996 9.12571C2.9362 8.06292 3.29424 7.01789 3.99634 6.21749C4.69844 5.4171 5.68793 4.92596 6.74996 4.85071C7.81199 4.92596 8.80148 5.4171 9.50358 6.21749C10.2057 7.01789 10.5637 8.06292 10.5 9.12571C10.5 9.52353 10.658 9.90506 10.9393 10.1864C11.2206 10.4677 11.6021 10.6257 12 10.6257C12.3978 10.6257 12.7793 10.4677 13.0606 10.1864C13.3419 9.90506 13.5 9.52353 13.5 9.12571C13.4362 8.06292 13.7942 7.01789 14.4963 6.21749C15.1984 5.4171 16.1879 4.92596 17.25 4.85071C18.312 4.92596 19.3015 5.4171 20.0036 6.21749C20.7057 7.01789 21.0637 8.06292 21 9.12571C21 11.7357 18.258 15.5307 13.477 19.5387Z"
                            fill="white"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_103">
                            <rect width="24" height="24" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-[#454545] text-2xl font-semibold ml-[15px]">
                      4.6
                    </p>
                  </div>
                </div>
                <div
                  class="absolute top-0 right-0 rounded-tr-[37.5px] rounded-bl-[42.5px] bg-[#39DB4A] px-6 py-8"
                >
                  <span class="inline-block"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <g clip-path="url(#clip0_1_103)">
                        <path
                          d="M17.25 1.85071C16.2243 1.86063 15.2152 2.11065 14.3035 2.58073C13.3918 3.05081 12.6029 3.72788 12 4.55771C11.397 3.72788 10.6081 3.05081 9.69644 2.58073C8.78476 2.11065 7.77565 1.86063 6.74996 1.85071C4.89173 1.92491 3.13848 2.73189 1.87358 4.09517C0.608672 5.45846 -0.0649657 7.26713 -4.03235e-05 9.12571C-4.03235e-05 13.6777 4.67396 18.5507 8.59996 21.8377C9.55329 22.6393 10.7589 23.0788 12.0045 23.0788C13.25 23.0788 14.4556 22.6393 15.409 21.8377C19.331 18.5507 24.009 13.6777 24.009 9.12571C24.0738 7.26563 23.399 5.45564 22.1322 4.0921C20.8653 2.72856 19.1098 1.9226 17.25 1.85071ZM13.477 19.5387C13.0634 19.8869 12.5401 20.0779 11.9995 20.0779C11.4588 20.0779 10.9355 19.8869 10.522 19.5387C5.74196 15.5307 2.99996 11.7357 2.99996 9.12571C2.9362 8.06292 3.29424 7.01789 3.99634 6.21749C4.69844 5.4171 5.68793 4.92596 6.74996 4.85071C7.81199 4.92596 8.80148 5.4171 9.50358 6.21749C10.2057 7.01789 10.5637 8.06292 10.5 9.12571C10.5 9.52353 10.658 9.90506 10.9393 10.1864C11.2206 10.4677 11.6021 10.6257 12 10.6257C12.3978 10.6257 12.7793 10.4677 13.0606 10.1864C13.3419 9.90506 13.5 9.52353 13.5 9.12571C13.4362 8.06292 13.7942 7.01789 14.4963 6.21749C15.1984 5.4171 16.1879 4.92596 17.25 4.85071C18.312 4.92596 19.3015 5.4171 20.0036 6.21749C20.7057 7.01789 21.0637 8.06292 21 9.12571C21 11.7357 18.258 15.5307 13.477 19.5387Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1_103">
                          <rect width="24" height="24" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </span>
                </div>
              </div>
            </div>
            <div class="swiper-slide">
              <div
                class="pt-[39px] pb-[46px] px-[40px] bg-white rounded-[40px] shadow-[2px_9px_42px_0px_rgba(0,0,0,0.08)] relative"
              >
                <div
                  class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
                >
                  <img src={Image10} alt="burger" />
                </div>
                <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                  Egg vegi salad
                </h4>
                <p class="text-[#555555] font-medium text-[22px] mt-3">
                  Description of the item
                </p>
                <div class="flex items-center justify-between mt-2.5">
                  <p class="text-2xl font-semibold text-[#515151]">
                    <span class="text-xl text-[#FF7979]">$</span>23.00
                  </p>
                  <div class="flex items-center">
                    <div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_97)">
                          <path
                            d="M1.3268 12.4003L4.8868 15.0003L3.5348 19.1873C3.31631 19.8367 3.31354 20.5393 3.52691 21.1904C3.74027 21.8415 4.15834 22.4062 4.7188 22.8003C5.26965 23.2071 5.93719 23.425 6.62195 23.4216C7.30671 23.4182 7.97204 23.1936 8.5188 22.7813L11.9998 20.2193L15.4818 22.7783C16.0317 23.1828 16.6956 23.4024 17.3782 23.4058C18.0607 23.4091 18.7268 23.1959 19.2806 22.7968C19.8343 22.3977 20.2473 21.8333 20.4601 21.1848C20.6729 20.5362 20.6745 19.8369 20.4648 19.1873L19.1128 15.0003L22.6728 12.4003C23.2219 11.9988 23.6302 11.4341 23.8391 10.7867C24.0481 10.1393 24.0472 9.44249 23.8365 8.79569C23.6258 8.1489 23.216 7.58523 22.6658 7.18521C22.1156 6.78519 21.4531 6.56927 20.7728 6.56831H16.3998L15.0728 2.43231C14.8641 1.78126 14.4541 1.21332 13.9018 0.810371C13.3495 0.407422 12.6835 0.190292 11.9998 0.190292C11.3161 0.190292 10.6501 0.407422 10.0978 0.810371C9.54553 1.21332 9.13548 1.78126 8.9268 2.43231L7.5998 6.56831H3.2308C2.55054 6.56927 1.88799 6.78519 1.33778 7.18521C0.787564 7.58523 0.377837 8.1489 0.167118 8.79569C-0.0436018 9.44249 -0.0445344 10.1393 0.164453 10.7867C0.37344 11.4341 0.781657 11.9988 1.3308 12.4003H1.3268Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_97">
                            <rect width="24" height="24" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-[#454545] text-2xl font-semibold ml-[15px]">
                      4.5
                    </p>
                  </div>
                </div>
                <div
                  class="absolute top-0 right-0 rounded-tr-[37.5px] rounded-bl-[42.5px] bg-[#39DB4A] px-6 py-8"
                >
                  <span class="inline-block"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <g clip-path="url(#clip0_1_103)">
                        <path
                          d="M17.25 1.85071C16.2243 1.86063 15.2152 2.11065 14.3035 2.58073C13.3918 3.05081 12.6029 3.72788 12 4.55771C11.397 3.72788 10.6081 3.05081 9.69644 2.58073C8.78476 2.11065 7.77565 1.86063 6.74996 1.85071C4.89173 1.92491 3.13848 2.73189 1.87358 4.09517C0.608672 5.45846 -0.0649657 7.26713 -4.03235e-05 9.12571C-4.03235e-05 13.6777 4.67396 18.5507 8.59996 21.8377C9.55329 22.6393 10.7589 23.0788 12.0045 23.0788C13.25 23.0788 14.4556 22.6393 15.409 21.8377C19.331 18.5507 24.009 13.6777 24.009 9.12571C24.0738 7.26563 23.399 5.45564 22.1322 4.0921C20.8653 2.72856 19.1098 1.9226 17.25 1.85071ZM13.477 19.5387C13.0634 19.8869 12.5401 20.0779 11.9995 20.0779C11.4588 20.0779 10.9355 19.8869 10.522 19.5387C5.74196 15.5307 2.99996 11.7357 2.99996 9.12571C2.9362 8.06292 3.29424 7.01789 3.99634 6.21749C4.69844 5.4171 5.68793 4.92596 6.74996 4.85071C7.81199 4.92596 8.80148 5.4171 9.50358 6.21749C10.2057 7.01789 10.5637 8.06292 10.5 9.12571C10.5 9.52353 10.658 9.90506 10.9393 10.1864C11.2206 10.4677 11.6021 10.6257 12 10.6257C12.3978 10.6257 12.7793 10.4677 13.0606 10.1864C13.3419 9.90506 13.5 9.52353 13.5 9.12571C13.4362 8.06292 13.7942 7.01789 14.4963 6.21749C15.1984 5.4171 16.1879 4.92596 17.25 4.85071C18.312 4.92596 19.3015 5.4171 20.0036 6.21749C20.7057 7.01789 21.0637 8.06292 21 9.12571C21 11.7357 18.258 15.5307 13.477 19.5387Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1_103">
                          <rect width="24" height="24" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </span>
                </div>
              </div>
            </div>
            <div class="swiper-slide">
              <div
                class="pt-[39px] pb-[46px] px-[40px] bg-white rounded-[40px] shadow-[2px_9px_42px_0px_rgba(0,0,0,0.08)] relative"
              >
                <div
                  class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
                >
                  <img src={Image11} alt="burger" />
                </div>
                <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                  Fattoush salad
                </h4>
                <p class="text-[#555555] font-medium text-[22px] mt-3">
                  Description of the item
                </p>
                <div class="flex items-center justify-between mt-2.5">
                  <p class="text-2xl font-semibold text-[#515151]">
                    <span class="text-xl text-[#FF7979]">$</span>24.00
                  </p>
                  <div class="flex items-center">
                    <div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_97)">
                          <path
                            d="M1.3268 12.4003L4.8868 15.0003L3.5348 19.1873C3.31631 19.8367 3.31354 20.5393 3.52691 21.1904C3.74027 21.8415 4.15834 22.4062 4.7188 22.8003C5.26965 23.2071 5.93719 23.425 6.62195 23.4216C7.30671 23.4182 7.97204 23.1936 8.5188 22.7813L11.9998 20.2193L15.4818 22.7783C16.0317 23.1828 16.6956 23.4024 17.3782 23.4058C18.0607 23.4091 18.7268 23.1959 19.2806 22.7968C19.8343 22.3977 20.2473 21.8333 20.4601 21.1848C20.6729 20.5362 20.6745 19.8369 20.4648 19.1873L19.1128 15.0003L22.6728 12.4003C23.2219 11.9988 23.6302 11.4341 23.8391 10.7867C24.0481 10.1393 24.0472 9.44249 23.8365 8.79569C23.6258 8.1489 23.216 7.58523 22.6658 7.18521C22.1156 6.78519 21.4531 6.56927 20.7728 6.56831H16.3998L15.0728 2.43231C14.8641 1.78126 14.4541 1.21332 13.9018 0.810371C13.3495 0.407422 12.6835 0.190292 11.9998 0.190292C11.3161 0.190292 10.6501 0.407422 10.0978 0.810371C9.54553 1.21332 9.13548 1.78126 8.9268 2.43231L7.5998 6.56831H3.2308C2.55054 6.56927 1.88799 6.78519 1.33778 7.18521C0.787564 7.58523 0.377837 8.1489 0.167118 8.79569C-0.0436018 9.44249 -0.0445344 10.1393 0.164453 10.7867C0.37344 11.4341 0.781657 11.9988 1.3308 12.4003H1.3268Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_97">
                            <rect width="24" height="24" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-[#454545] text-2xl font-semibold ml-[15px]">
                      4.9
                    </p>
                  </div>
                </div>
                <div
                  class="absolute top-0 right-0 rounded-tr-[37.5px] rounded-bl-[42.5px] bg-[#39DB4A] px-6 py-8"
                >
                  <span class="inline-block"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <g clip-path="url(#clip0_1_105)">
                        <path
                          d="M17.5 1.91653C16.3739 1.93405 15.2724 2.24839 14.3068 2.82781C13.3411 3.40722 12.5453 4.2312 12 5.21653C11.4546 4.2312 10.6589 3.40722 9.69323 2.82781C8.72753 2.24839 7.62604 1.93405 6.49999 1.91653C4.70493 1.99453 3.01369 2.77979 1.79577 4.10077C0.577848 5.42175 -0.0677617 7.17106 -1.11917e-05 8.96653C-1.11917e-05 13.5135 4.78599 18.4795 8.79999 21.8465C9.69621 22.5997 10.8293 23.0126 12 23.0126C13.1706 23.0126 14.3038 22.5997 15.2 21.8465C19.214 18.4795 24 13.5135 24 8.96653C24.0677 7.17106 23.4221 5.42175 22.2042 4.10077C20.9863 2.77979 19.295 1.99453 17.5 1.91653Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1_105">
                          <rect width="24" height="24" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </span>
                </div>
              </div>
            </div>
            <div class="swiper-slide">
              <div
                class="pt-[39px] pb-[46px] px-[40px] bg-white rounded-[40px] shadow-[2px_9px_42px_0px_rgba(0,0,0,0.08)] relative"
              >
                <div
                  class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
                >
                  <img src={Image12} alt="burger" />
                </div>
                <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                  Vegetable salad
                </h4>
                <p class="text-[#555555] font-medium text-[22px] mt-3">
                  Description of the item
                </p>
                <div class="flex items-center justify-between mt-2.5">
                  <p class="text-2xl font-semibold text-[#515151]">
                    <span class="text-xl text-[#FF7979]">$</span>26.00
                  </p>
                  <div class="flex items-center">
                    <div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_103)">
                          <path
                            d="M17.25 1.85071C16.2243 1.86063 15.2152 2.11065 14.3035 2.58073C13.3918 3.05081 12.6029 3.72788 12 4.55771C11.397 3.72788 10.6081 3.05081 9.69644 2.58073C8.78476 2.11065 7.77565 1.86063 6.74996 1.85071C4.89173 1.92491 3.13848 2.73189 1.87358 4.09517C0.608672 5.45846 -0.0649657 7.26713 -4.03235e-05 9.12571C-4.03235e-05 13.6777 4.67396 18.5507 8.59996 21.8377C9.55329 22.6393 10.7589 23.0788 12.0045 23.0788C13.25 23.0788 14.4556 22.6393 15.409 21.8377C19.331 18.5507 24.009 13.6777 24.009 9.12571C24.0738 7.26563 23.399 5.45564 22.1322 4.0921C20.8653 2.72856 19.1098 1.9226 17.25 1.85071ZM13.477 19.5387C13.0634 19.8869 12.5401 20.0779 11.9995 20.0779C11.4588 20.0779 10.9355 19.8869 10.522 19.5387C5.74196 15.5307 2.99996 11.7357 2.99996 9.12571C2.9362 8.06292 3.29424 7.01789 3.99634 6.21749C4.69844 5.4171 5.68793 4.92596 6.74996 4.85071C7.81199 4.92596 8.80148 5.4171 9.50358 6.21749C10.2057 7.01789 10.5637 8.06292 10.5 9.12571C10.5 9.52353 10.658 9.90506 10.9393 10.1864C11.2206 10.4677 11.6021 10.6257 12 10.6257C12.3978 10.6257 12.7793 10.4677 13.0606 10.1864C13.3419 9.90506 13.5 9.52353 13.5 9.12571C13.4362 8.06292 13.7942 7.01789 14.4963 6.21749C15.1984 5.4171 16.1879 4.92596 17.25 4.85071C18.312 4.92596 19.3015 5.4171 20.0036 6.21749C20.7057 7.01789 21.0637 8.06292 21 9.12571C21 11.7357 18.258 15.5307 13.477 19.5387Z"
                            fill="white"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_103">
                            <rect width="24" height="24" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-[#454545] text-2xl font-semibold ml-[15px]">
                      4.6
                    </p>
                  </div>
                </div>
                <div
                  class="absolute top-0 right-0 rounded-tr-[37.5px] rounded-bl-[42.5px] bg-[#39DB4A] px-6 py-8"
                >
                  <span class="inline-block"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <g clip-path="url(#clip0_1_103)">
                        <path
                          d="M17.25 1.85071C16.2243 1.86063 15.2152 2.11065 14.3035 2.58073C13.3918 3.05081 12.6029 3.72788 12 4.55771C11.397 3.72788 10.6081 3.05081 9.69644 2.58073C8.78476 2.11065 7.77565 1.86063 6.74996 1.85071C4.89173 1.92491 3.13848 2.73189 1.87358 4.09517C0.608672 5.45846 -0.0649657 7.26713 -4.03235e-05 9.12571C-4.03235e-05 13.6777 4.67396 18.5507 8.59996 21.8377C9.55329 22.6393 10.7589 23.0788 12.0045 23.0788C13.25 23.0788 14.4556 22.6393 15.409 21.8377C19.331 18.5507 24.009 13.6777 24.009 9.12571C24.0738 7.26563 23.399 5.45564 22.1322 4.0921C20.8653 2.72856 19.1098 1.9226 17.25 1.85071ZM13.477 19.5387C13.0634 19.8869 12.5401 20.0779 11.9995 20.0779C11.4588 20.0779 10.9355 19.8869 10.522 19.5387C5.74196 15.5307 2.99996 11.7357 2.99996 9.12571C2.9362 8.06292 3.29424 7.01789 3.99634 6.21749C4.69844 5.4171 5.68793 4.92596 6.74996 4.85071C7.81199 4.92596 8.80148 5.4171 9.50358 6.21749C10.2057 7.01789 10.5637 8.06292 10.5 9.12571C10.5 9.52353 10.658 9.90506 10.9393 10.1864C11.2206 10.4677 11.6021 10.6257 12 10.6257C12.3978 10.6257 12.7793 10.4677 13.0606 10.1864C13.3419 9.90506 13.5 9.52353 13.5 9.12571C13.4362 8.06292 13.7942 7.01789 14.4963 6.21749C15.1984 5.4171 16.1879 4.92596 17.25 4.85071C18.312 4.92596 19.3015 5.4171 20.0036 6.21749C20.7057 7.01789 21.0637 8.06292 21 9.12571C21 11.7357 18.258 15.5307 13.477 19.5387Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1_103">
                          <rect width="24" height="24" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </span>
                </div>
              </div>
            </div>
            <div class="swiper-slide">
              <div
                class="pt-[39px] pb-[46px] px-[40px] bg-white rounded-[40px] shadow-[2px_9px_42px_0px_rgba(0,0,0,0.08)] relative"
              >
                <div
                  class="size-[153px] rounded-full bg-[#C1F1C6] mx-auto flex justify-center items-center"
                >
                  <img src={Image13} alt="burger" />
                </div>
                <h4 class="text-[#1E1E1E] font-semibold text-2xl mt-[13px]">
                  Egg vegi salad
                </h4>
                <p class="text-[#555555] font-medium text-[22px] mt-3">
                  Description of the item
                </p>
                <div class="flex items-center justify-between mt-2.5">
                  <p class="text-2xl font-semibold text-[#515151]">
                    <span class="text-xl text-[#FF7979]">$</span>23.00
                  </p>
                  <div class="flex items-center">
                    <div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_103)">
                          <path
                            d="M17.25 1.85071C16.2243 1.86063 15.2152 2.11065 14.3035 2.58073C13.3918 3.05081 12.6029 3.72788 12 4.55771C11.397 3.72788 10.6081 3.05081 9.69644 2.58073C8.78476 2.11065 7.77565 1.86063 6.74996 1.85071C4.89173 1.92491 3.13848 2.73189 1.87358 4.09517C0.608672 5.45846 -0.0649657 7.26713 -4.03235e-05 9.12571C-4.03235e-05 13.6777 4.67396 18.5507 8.59996 21.8377C9.55329 22.6393 10.7589 23.0788 12.0045 23.0788C13.25 23.0788 14.4556 22.6393 15.409 21.8377C19.331 18.5507 24.009 13.6777 24.009 9.12571C24.0738 7.26563 23.399 5.45564 22.1322 4.0921C20.8653 2.72856 19.1098 1.9226 17.25 1.85071ZM13.477 19.5387C13.0634 19.8869 12.5401 20.0779 11.9995 20.0779C11.4588 20.0779 10.9355 19.8869 10.522 19.5387C5.74196 15.5307 2.99996 11.7357 2.99996 9.12571C2.9362 8.06292 3.29424 7.01789 3.99634 6.21749C4.69844 5.4171 5.68793 4.92596 6.74996 4.85071C7.81199 4.92596 8.80148 5.4171 9.50358 6.21749C10.2057 7.01789 10.5637 8.06292 10.5 9.12571C10.5 9.52353 10.658 9.90506 10.9393 10.1864C11.2206 10.4677 11.6021 10.6257 12 10.6257C12.3978 10.6257 12.7793 10.4677 13.0606 10.1864C13.3419 9.90506 13.5 9.52353 13.5 9.12571C13.4362 8.06292 13.7942 7.01789 14.4963 6.21749C15.1984 5.4171 16.1879 4.92596 17.25 4.85071C18.312 4.92596 19.3015 5.4171 20.0036 6.21749C20.7057 7.01789 21.0637 8.06292 21 9.12571C21 11.7357 18.258 15.5307 13.477 19.5387Z"
                            fill="white"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_103">
                            <rect width="24" height="24" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <p class="text-[#454545] text-2xl font-semibold ml-[15px]">
                      4.5
                    </p>
                  </div>
                </div>
                <div
                  class="absolute top-0 right-0 rounded-tr-[37.5px] rounded-bl-[42.5px] bg-[#39DB4A] px-6 py-8"
                >
                  <span class="inline-block"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <g clip-path="url(#clip0_1_105)">
                        <path
                          d="M17.5 1.91653C16.3739 1.93405 15.2724 2.24839 14.3068 2.82781C13.3411 3.40722 12.5453 4.2312 12 5.21653C11.4546 4.2312 10.6589 3.40722 9.69323 2.82781C8.72753 2.24839 7.62604 1.93405 6.49999 1.91653C4.70493 1.99453 3.01369 2.77979 1.79577 4.10077C0.577848 5.42175 -0.0677617 7.17106 -1.11917e-05 8.96653C-1.11917e-05 13.5135 4.78599 18.4795 8.79999 21.8465C9.69621 22.5997 10.8293 23.0126 12 23.0126C13.1706 23.0126 14.3038 22.5997 15.2 21.8465C19.214 18.4795 24 13.5135 24 8.96653C24.0677 7.17106 23.4221 5.42175 22.2042 4.10077C20.9863 2.77979 19.295 1.99453 17.5 1.91653Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1_105">
                          <rect width="24" height="24" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="py-12 md:py-20 lg:py-24">
      <div class="main-wrapper">
        <div class="flex flex-wrap -mx-4 gap-y-6 items-center">
          <div class="xl:w-6/12 lg:w-6/12 md:w-6/12 sm:w-full w-full px-4">
            <div class="relative">
              <img src={Image14} alt="#" />
              <div
                class="shadow-[26px_22px_51px_rgba(0,0,0,0.14)] absolute -bottom-4 right-0"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="230"
                  height="71"
                  viewBox="0 0 230 71"
                  fill="none"
                >
                  <path
                    d="M1.22059 5.7879C1.51428 0.592527 7.88736 -1.72255 11.4471 2.07303C11.814 2.46424 12.2324 2.80377 12.6907 3.08229L17.4911 5.99916C24.8554 10.474 33.3072 12.8405 41.9245 12.8405H57.5H115H200.92C216.981 12.8405 230 25.8599 230 41.9202C230 57.9806 216.981 71 200.92 71H29.0798C13.0195 71 0 57.9806 0 41.9202V27.3804L1.22059 5.7879Z"
                    fill="white"
                  />
                </svg>
                <span
                  class="-mt-[20px] text-[#FF6868] text-xl font-semibold inline-block absolute top-10 left-10"
                  >Our Best Chef 😁</span
                >
              </div>
            </div>
          </div>
          <div class="xl:w-6/12 lg:w-6/12 md:w-6/12 sm:w-full w-full px-4">
            <p class="text-[#FF6868] md:text-xl text-lg font-bold leading-tight">
              Testimonials
            </p>
            <h2
              class="font-extrabold text-black md:text-5xl text-[32px] mt-[53px]"
            >
              What Our Customers Say About Us
            </h2>
            <p class="text-[#555555] font-medium md:text-xl text-lg mt-[29px]">
              “I had the pleasure of dining at Foodi last night, and I'm still
              raving about the experience! The attention to detail in
              presentation and service was impeccable”
            </p>
            <div
              class="flex justify-between items-center mt-16 flex-wrap md:flex-nowrap gap-y-6"
            >
              <ul class="flex">
                <li>
                  <img
                    src={Image15}
                    alt="img-1"
                    class="rounded-full border-4 border-white"
                  />
                </li>
                <li class="-ml-10">
                  <img
                    src={Image16}
                    alt="img-1"
                    class="rounded-full border-4 border-white"
                  />
                </li>
                <li class="-ml-10">
                  <img
                    src={Image17}
                    alt="img-1"
                    class="rounded-full border-4 border-white"
                  />
                </li>
              </ul>
              <div>
                <h4 class="text-black text-2xl font-semibold">
                  Customer Feedback
                </h4>
                <p class="text-[#807E7E] text-2xl font-medium">
                  <span class="text-[#454545] text-2xl font-semibold mr-4"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                      class="mr-3.5 inline-block"
                    >
                      <g clip-path="url(#clip0_1_69)">
                        <path
                          d="M1.3268 12.4003L4.8868 15.0003L3.5348 19.1873C3.31631 19.8367 3.31354 20.5393 3.52691 21.1904C3.74027 21.8415 4.15834 22.4062 4.7188 22.8003C5.26965 23.2071 5.93719 23.425 6.62195 23.4216C7.30671 23.4182 7.97204 23.1936 8.5188 22.7813L11.9998 20.2193L15.4818 22.7783C16.0317 23.1828 16.6956 23.4025 17.3782 23.4058C18.0607 23.4091 18.7268 23.1959 19.2806 22.7968C19.8343 22.3978 20.2473 21.8334 20.4601 21.1848C20.6729 20.5362 20.6745 19.8369 20.4648 19.1873L19.1128 15.0003L22.6728 12.4003C23.2219 11.9988 23.6302 11.4341 23.8391 10.7867C24.0481 10.1394 24.0472 9.44251 23.8365 8.79571C23.6258 8.14891 23.216 7.58525 22.6658 7.18523C22.1156 6.7852 21.4531 6.56929 20.7728 6.56832H16.3998L15.0728 2.43232C14.8641 1.78128 14.4541 1.21333 13.9018 0.810386C13.3495 0.407437 12.6835 0.190308 11.9998 0.190308C11.3161 0.190308 10.6501 0.407437 10.0978 0.810386C9.54553 1.21333 9.13548 1.78128 8.9268 2.43232L7.5998 6.56832H3.2308C2.55054 6.56929 1.88799 6.7852 1.33778 7.18523C0.787564 7.58525 0.377837 8.14891 0.167118 8.79571C-0.0436018 9.44251 -0.0445344 10.1394 0.164453 10.7867C0.37344 11.4341 0.781657 11.9988 1.3308 12.4003H1.3268Z"
                          fill="#FFE605"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1_69">
                          <rect width="24" height="24" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                    4.9</span
                  >(18.6k Reviews)
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="py-[50px] md:py-[80px] lg:py-[100px]">
      <div class="main-wrapper">
        <div class="flex flex-wrap -mx-4 gap-y-6 gap-y-6 items-center">
          <div class="xl:w-6/12 lg:w-6/12 md:w-6/12 sm:w-full w-full px-4">
            <p class="text-[#FF6868] text-lg font-bold mb-[53px]">
              Our Story & Services
            </p>
            <h2 class="text-black md:text-5xl text-[32px] font-bold">
              Our Culinary Journey And Services
            </h2>

            <p class="text-[#555555] md:text-xl text-lg font-medium mt-[30px]">
              Rooted in passion, we curate unforgettable dining experiences and
              offer exceptional services, blending culinary artistry with warm
              hospitality.
            </p>
            <a
              href="#"
              class="inline-block bg-[#39DB4A] py-3 md:py-4 px-6 md:px-8  text-xl lg:text-2xl rounded-full text-white  font-semibold mt-5 border-2 border-[#39DB4A] hover:bg-white hover:text-[#39DB4A] hover:border-[#39DB4A]"
              >Explore</a
            >
          </div>
          <div class="xl:w-6/12 lg:w-6/12 md:w-6/12 sm:w-full w-full px-4">
            <div class="sm:flex  md:gap-x-2 lg:gap-x-9">
              <div
                class="px-7 pt-11 pb-[38px] bg-white shadow-[7px_12px_43px_0px_rgba(0,0,0,0.14)] rounded-[30px] text-center w-full sm:w-[277px] mt-8 sm:mt-0"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="64"
                    height="64"
                    viewBox="0 0 64 64"
                    fill="none"
                    class="mx-auto"
                  >
                    <g clip-path="url(#clip0_1_194)">
                      <path
                        d="M61.8503 30.9339C60.4334 29.0417 58.4201 27.6824 56.1356 27.0752C57.2491 23.8384 58.0719 20.5088 58.5942 17.1259C58.7649 15.8962 58.6479 14.6437 58.2522 13.4669C57.8566 12.2902 57.1932 11.2213 56.3143 10.3445C55.4355 9.46769 54.365 8.80684 53.1873 8.41398C52.0097 8.02113 50.7569 7.907 49.5276 8.08058C49.1916 8.12591 48.1943 8.27258 46.8316 8.53658C47.6754 6.66846 48.0751 4.63035 47.9996 2.58191C47.9884 2.23172 47.9083 1.88716 47.7639 1.56792C47.6196 1.24867 47.4137 0.960986 47.1582 0.721287C46.9026 0.481588 46.6024 0.29457 46.2746 0.17091C45.9467 0.047251 45.5978 -0.010628 45.2476 0.000578124C44.8974 0.0117843 44.5528 0.091856 44.2336 0.236222C43.9143 0.380587 43.6267 0.58642 43.387 0.841967C43.1473 1.09751 42.9602 1.39777 42.8366 1.72559C42.7129 2.05342 42.655 2.40239 42.6662 2.75258C42.7117 4.04076 42.4474 5.32104 41.8956 6.48591C41.5099 5.44499 40.9598 4.47265 40.2663 3.60591C39.0874 2.2643 37.591 1.23965 35.914 0.625654C34.2369 0.0116595 32.4327 -0.172078 30.6662 0.0912449C28.7294 0.246714 26.8781 0.955768 25.3329 2.13391C23.5861 0.819761 21.4728 0.0833252 19.2876 0.0272448C16.5147 -0.241501 13.7484 0.599431 11.5942 2.36591C10.5005 3.3397 9.61734 4.52686 8.99917 5.85446C8.381 7.18205 8.04084 8.62203 7.99958 10.0859C7.9264 12.43 8.60888 14.7354 9.94625 16.6619C8.31931 18.0568 7.0564 19.8268 6.26657 21.8191C5.47675 23.8113 5.18376 25.9658 5.41291 28.0966C3.30399 29.2718 1.66049 31.133 0.755335 33.3712C-0.149824 35.6093 -0.262185 38.0898 0.436911 40.4006C2.2636 46.7566 5.66352 52.5494 10.3222 57.2433C14.7916 61.6012 20.7946 64.0281 27.0369 64.0006H36.9622C43.2038 64.0285 49.2062 61.6015 53.6743 57.2433C58.3347 52.5503 61.7356 46.7572 63.5623 40.4006C64.0383 38.7988 64.1315 37.1076 63.8341 35.4633C63.5367 33.8189 62.8572 32.2675 61.8503 30.9339ZM50.2689 13.3606C50.6816 13.3049 51.1016 13.3454 51.496 13.4789C51.8905 13.6123 52.2487 13.8352 52.5428 14.13C52.8368 14.4249 53.0588 14.7837 53.1912 15.1785C53.3236 15.5733 53.363 15.9934 53.3063 16.4059C52.747 19.9077 51.8403 23.3452 50.5996 26.6672H43.7702L47.2183 23.2192C47.704 22.7163 47.9728 22.0427 47.9667 21.3435C47.9606 20.6443 47.6802 19.9755 47.1858 19.4811C46.6913 18.9866 46.0225 18.7062 45.3233 18.7001C44.6241 18.694 43.9505 18.9628 43.4476 19.4486L36.2289 26.6672H32.0982C31.9035 25.2647 32.0372 23.8361 32.4889 22.4941C32.9405 21.152 33.6978 19.9333 34.7009 18.9339C35.8396 17.7846 40.2049 14.6672 50.2689 13.3606ZM13.3329 10.2646C13.3489 9.53044 13.5145 8.80727 13.8194 8.13924C14.1243 7.47122 14.5621 6.87236 15.1062 6.37925C16.2064 5.56603 17.5732 5.19838 18.9329 5.34991C19.5897 5.32291 20.2447 5.43511 20.855 5.67915C21.4654 5.92318 22.0172 6.29354 22.4742 6.76591C22.8341 7.16795 23.2747 7.48953 23.7673 7.70964C24.2599 7.92974 24.7934 8.04342 25.3329 8.04325C25.8712 8.04059 26.4029 7.92478 26.8936 7.70335C27.3842 7.48192 27.8228 7.15981 28.1809 6.75791C28.5786 6.3372 29.0559 5.99964 29.5851 5.7648C30.1142 5.52997 30.6848 5.40254 31.2636 5.38991C32.1451 5.2329 33.0517 5.29686 33.9024 5.57609C34.7531 5.85531 35.5213 6.34107 36.1382 6.98991C36.9478 8.0263 37.3703 9.31269 37.3329 10.6272C37.3329 10.8006 37.3329 11.1792 37.3329 11.1792C33.7929 12.454 30.7689 14.855 28.7249 18.0139C27.041 16.0205 24.8006 14.575 22.2903 13.8624C19.7801 13.1498 17.1143 13.2026 14.6342 14.0139C13.7493 12.9695 13.2853 11.6327 13.3329 10.2646ZM10.6662 26.6672C10.6662 24.5455 11.5091 22.5107 13.0094 21.0104C14.5097 19.5101 16.5445 18.6672 18.6662 18.6672C20.788 18.6672 22.8228 19.5101 24.3231 21.0104C25.8234 22.5107 26.6662 24.5455 26.6662 26.6672H10.6662ZM58.4423 38.9046C56.8489 44.3903 53.9165 49.3938 49.9089 53.4646C46.4438 56.8313 41.7935 58.7 36.9622 58.6673H27.0369C22.2043 58.6988 17.5528 56.8304 14.0849 53.4646C10.0771 49.3939 7.14469 44.3904 5.55158 38.9046C5.31251 38.0972 5.26615 37.245 5.4162 36.4165C5.56625 35.588 5.90855 34.8062 6.41558 34.1339C6.90755 33.4703 7.54851 32.9315 8.28683 32.561C9.02514 32.1904 9.84017 31.9985 10.6662 32.0006H53.3329C54.1617 31.996 54.9799 32.1867 55.7212 32.5574C56.4625 32.928 57.106 33.4681 57.5996 34.1339C58.103 34.808 58.4415 35.5906 58.5878 36.4191C58.7341 37.2476 58.6843 38.0988 58.4423 38.9046Z"
                        fill="#5FE26C"
                      />
                    </g>
                    <defs>
                      <clipPath id="clip0_1_194">
                        <rect width="64" height="64" fill="white" />
                      </clipPath>
                    </defs>
                  </svg>
                </div>
                <h5 class="text-[#5FE26C] text-lg font-bold mt-4 mb-4">
                  Catering
                </h5>
                <p class="text-[#90BD95] text-sm font-semibold">
                  Delight your guests with our flavors and presentation
                </p>
              </div>
              <div
                class="px-7 pt-11 pb-[38px] bg-white shadow-[7px_12px_43px_0px_rgba(0,0,0,0.14)] rounded-[30px] text-center w-full sm:w-[277px] mt-8 sm:mt-0"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="64"
                    height="64"
                    viewBox="0 0 64 64"
                    fill="none"
                    class="mx-auto"
                  >
                    <g clip-path="url(#clip0_1_193)">
                      <path
                        d="M24 63.9983H2.66667C1.95942 63.9983 1.28115 63.7174 0.781049 63.2173C0.280952 62.7172 0 62.0389 0 61.3317C0 60.6244 0.280952 59.9462 0.781049 59.4461C1.28115 58.946 1.95942 58.665 2.66667 58.665H24C24.7072 58.665 25.3855 58.946 25.8856 59.4461C26.3857 59.9462 26.6667 60.6244 26.6667 61.3317C26.6667 62.0389 26.3857 62.7172 25.8856 63.2173C25.3855 63.7174 24.7072 63.9983 24 63.9983Z"
                        fill="#5FE26C"
                      />
                      <path
                        d="M18.6667 53.3334H2.66667C1.95942 53.3334 1.28115 53.0524 0.781049 52.5523C0.280952 52.0522 0 51.3739 0 50.6667C0 49.9594 0.280952 49.2812 0.781049 48.7811C1.28115 48.281 1.95942 48 2.66667 48H18.6667C19.3739 48 20.0522 48.281 20.5523 48.7811C21.0524 49.2812 21.3333 49.9594 21.3333 50.6667C21.3333 51.3739 21.0524 52.0522 20.5523 52.5523C20.0522 53.0524 19.3739 53.3334 18.6667 53.3334Z"
                        fill="#5FE26C"
                      />
                      <path
                        d="M13.3333 42.6683H2.66667C1.95942 42.6683 1.28115 42.3874 0.781049 41.8873C0.280952 41.3872 0 40.7089 0 40.0016C0 39.2944 0.280952 38.6161 0.781049 38.116C1.28115 37.6159 1.95942 37.335 2.66667 37.335H13.3333C14.0406 37.335 14.7189 37.6159 15.219 38.116C15.719 38.6161 16 39.2944 16 40.0016C16 40.7089 15.719 41.3872 15.219 41.8873C14.7189 42.3874 14.0406 42.6683 13.3333 42.6683Z"
                        fill="#5FE26C"
                      />
                      <path
                        d="M34.6671 63.8788C33.9599 63.9103 33.2691 63.6595 32.7468 63.1817C32.2244 62.7039 31.9133 62.0381 31.8818 61.3308C31.8503 60.6236 32.1011 59.9328 32.579 59.4105C33.0568 58.8881 33.7226 58.577 34.4298 58.5455C39.4764 58.0838 44.2869 56.1936 48.2979 53.0964C52.3089 49.9992 55.3544 45.8232 57.0776 41.0576C58.8008 36.2919 59.1304 31.1339 58.0278 26.1877C56.9252 21.2414 54.436 16.7118 50.8518 13.1292C47.2676 9.54671 42.7369 7.05963 37.7901 5.95929C32.8434 4.85896 27.6855 5.19093 22.9207 6.91633C18.1558 8.64174 13.9813 11.6891 10.8859 15.7016C7.79057 19.714 5.90261 24.5254 5.44318 29.5722C5.37953 30.2766 5.03866 30.9269 4.49555 31.3799C3.95245 31.833 3.2516 32.0518 2.54719 31.9882C1.84277 31.9245 1.1925 31.5836 0.739408 31.0405C0.286321 30.4974 0.0675382 29.7966 0.13119 29.0922C0.874893 20.8877 4.75413 13.286 10.9614 7.86968C17.1686 2.45332 25.2257 -0.360454 33.4553 0.0140732C41.6849 0.3886 49.453 3.92257 55.1427 9.88035C60.8323 15.8381 64.005 23.7607 64.0004 31.9988C64.0413 39.9944 61.0709 47.7124 55.6802 53.6175C50.2894 59.5227 42.8733 63.1822 34.9072 63.8682C34.8271 63.8762 34.7445 63.8788 34.6671 63.8788Z"
                        fill="#5FE26C"
                      />
                      <path
                        d="M32.0004 16C31.2932 16 30.6149 16.281 30.1148 16.781C29.6147 17.2811 29.3337 17.9594 29.3337 18.6667V32C29.3339 32.7072 29.6149 33.3854 30.1151 33.8853L38.1151 41.8853C38.618 42.3711 39.2916 42.6399 39.9908 42.6338C40.69 42.6277 41.3588 42.3473 41.8533 41.8528C42.3477 41.3584 42.6281 40.6896 42.6342 39.9904C42.6403 39.2912 42.3715 38.6176 41.8857 38.1147L34.6671 30.896V18.6667C34.6671 17.9594 34.3861 17.2811 33.886 16.781C33.3859 16.281 32.7077 16 32.0004 16Z"
                        fill="#5FE26C"
                      />
                    </g>
                    <defs>
                      <clipPath id="clip0_1_193">
                        <rect width="64" height="64" fill="white" />
                      </clipPath>
                    </defs>
                  </svg>
                </div>
                <h5 class="text-[#5FE26C] text-lg font-bold mt-4 mb-4">
                  Fast delivery
                </h5>
                <p class="text-[#90BD95] text-sm font-semibold">
                  We deliver your order promptly to your door
                </p>
              </div>
            </div>
            <div class="sm:flex md:gap-x-2 lg:gap-x-9 sm:mt-8">
              <div
                class="px-7 pt-11 pb-[38px] bg-white shadow-[7px_12px_43px_0px_rgba(0,0,0,0.14)] rounded-[30px] text-center w-full sm:w-[277px] mt-8 sm:mt-0"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="64"
                    height="64"
                    viewBox="0 0 64 64"
                    fill="none"
                    class="mx-auto"
                  >
                    <g clip-path="url(#clip0_1_196)">
                      <path
                        d="M18.6664 64.0017C21.612 64.0017 23.9998 61.6139 23.9998 58.6684C23.9998 55.7229 21.612 53.3351 18.6664 53.3351C15.7209 53.3351 13.3331 55.7229 13.3331 58.6684C13.3331 61.6139 15.7209 64.0017 18.6664 64.0017Z"
                        fill="#5FE26C"
                      />
                      <path
                        d="M45.3333 64.0017C48.2788 64.0017 50.6666 61.6139 50.6666 58.6684C50.6666 55.7229 48.2788 53.3351 45.3333 53.3351C42.3878 53.3351 40 55.7229 40 58.6684C40 61.6139 42.3878 64.0017 45.3333 64.0017Z"
                        fill="#5FE26C"
                      />
                      <path
                        d="M63.1598 3.56104C62.6597 3.06111 61.9815 2.78027 61.2744 2.78027C60.5673 2.78027 59.8892 3.06111 59.3891 3.56104L45.6318 17.3317L41.4958 13.0144C41.2531 12.7619 40.963 12.5597 40.6422 12.4193C40.3214 12.2789 39.976 12.203 39.6259 12.1961C39.2758 12.1892 38.9277 12.2513 38.6015 12.3789C38.2754 12.5064 37.9776 12.697 37.7251 12.9397C37.4726 13.1824 37.2704 13.4724 37.13 13.7933C36.9896 14.1141 36.9138 14.4594 36.9068 14.8096C36.8928 15.5167 37.1603 16.2004 37.6504 16.7104L41.9544 21.1877C42.413 21.683 42.9673 22.0802 43.5838 22.3552C44.2003 22.6302 44.8661 22.7774 45.5411 22.7877H45.6291C46.2906 22.7899 46.9459 22.6607 47.557 22.4075C48.1682 22.1544 48.7229 21.7823 49.1891 21.313L63.1598 7.3317C63.6597 6.83163 63.9405 6.15348 63.9405 5.44637C63.9405 4.73927 63.6597 4.06111 63.1598 3.56104Z"
                        fill="#5FE26C"
                      />
                      <path
                        d="M58.4 24.0427C58.0552 23.9804 57.7015 23.9866 57.3592 24.0611C57.0168 24.1355 56.6925 24.2767 56.4047 24.4766C56.117 24.6764 55.8714 24.931 55.6821 25.2258C55.4927 25.5206 55.3634 25.8498 55.3013 26.1947L54.96 28.0853C54.6273 29.9315 53.6565 31.6021 52.2171 32.8051C50.7776 34.0081 48.9613 34.667 47.0853 34.6667H14.448L11.9413 13.3333H29.3333C30.0406 13.3333 30.7189 13.0524 31.2189 12.5523C31.719 12.0522 32 11.3739 32 10.6667C32 9.95942 31.719 9.28115 31.2189 8.78105C30.7189 8.28095 30.0406 8 29.3333 8H11.312L11.2 7.06133C10.9702 5.11626 10.0348 3.32308 8.57117 2.02161C7.10751 0.720145 5.21727 0.000834231 3.25867 0L2.66667 0C1.95942 0 1.28115 0.280952 0.781049 0.781049C0.280952 1.28115 0 1.95942 0 2.66667C0 3.37391 0.280952 4.05219 0.781049 4.55229C1.28115 5.05238 1.95942 5.33333 2.66667 5.33333H3.25867C3.91182 5.33342 4.54223 5.57322 5.03033 6.00724C5.51842 6.44127 5.83025 7.03933 5.90667 7.688L9.576 38.888C9.95694 42.1328 11.5159 45.1248 13.9571 47.296C16.3982 49.4673 19.5516 50.6667 22.8187 50.6667H50.6667C51.3739 50.6667 52.0522 50.3857 52.5523 49.8856C53.0524 49.3855 53.3333 48.7072 53.3333 48C53.3333 47.2928 53.0524 46.6145 52.5523 46.1144C52.0522 45.6143 51.3739 45.3333 50.6667 45.3333H22.8187C21.164 45.3337 19.55 44.8209 18.199 43.8657C16.8479 42.9106 15.8262 41.56 15.2747 40H47.0853C50.2114 40.0002 53.2382 38.902 55.6369 36.8972C58.0355 34.8924 59.6534 32.1085 60.208 29.032L60.5493 27.1387C60.6744 26.4431 60.5183 25.7264 60.1153 25.1459C59.7123 24.5654 59.0954 24.1686 58.4 24.0427Z"
                        fill="#5FE26C"
                      />
                    </g>
                    <defs>
                      <clipPath id="clip0_1_196">
                        <rect width="64" height="64" fill="white" />
                      </clipPath>
                    </defs>
                  </svg>
                </div>
                <h5 class="text-[#5FE26C] text-lg font-bold mt-4 mb-4">
                  Online Ordering
                </h5>
                <p class="text-[#90BD95] text-sm font-semibold">
                  Explore menu & order with ease using our Online Ordering
                </p>
              </div>
              <div
                class="px-7 pt-11 pb-[38px] bg-white shadow-[7px_12px_43px_0px_rgba(0,0,0,0.14)] rounded-[30px] text-center w-full sm:w-[277px] mt-8 sm:mt-0"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="64"
                    height="64"
                    viewBox="0 0 64 64"
                    fill="none"
                    class="mx-auto"
                  >
                    <g clip-path="url(#clip0_1_195)">
                      <path
                        d="M53.3333 18.6667H48.6987C50.2064 17.3376 51.4032 15.693 52.2042 13.8496C53.0051 12.0062 53.3906 10.0091 53.3333 8C53.3333 7.29276 53.0524 6.61448 52.5523 6.11438C52.0522 5.61428 51.3739 5.33333 50.6667 5.33333C49.9594 5.33333 49.2811 5.61428 48.781 6.11438C48.281 6.61448 48 7.29276 48 8C48 14.992 41.6773 17.4133 36.8693 18.2427C38.644 15.0961 39.7122 11.601 40 8C40 5.87827 39.1571 3.84344 37.6569 2.34315C36.1566 0.842855 34.1217 0 32 0C29.8783 0 27.8434 0.842855 26.3431 2.34315C24.8429 3.84344 24 5.87827 24 8C24.2878 11.601 25.356 15.0961 27.1307 18.2427C22.3227 17.4133 16 14.992 16 8C16 7.29276 15.719 6.61448 15.219 6.11438C14.7189 5.61428 14.0406 5.33333 13.3333 5.33333C12.6261 5.33333 11.9478 5.61428 11.4477 6.11438C10.9476 6.61448 10.6667 7.29276 10.6667 8C10.6094 10.0091 10.9949 12.0062 11.7958 13.8496C12.5968 15.693 13.7936 17.3376 15.3013 18.6667H10.6667C7.83769 18.6667 5.12458 19.7905 3.12419 21.7909C1.12381 23.7912 0 26.5044 0 29.3333L0 32C0 33.4145 0.561903 34.771 1.5621 35.7712C2.56229 36.7714 3.91885 37.3333 5.33333 37.3333V50.6667C5.33757 54.2016 6.74369 57.5905 9.24325 60.0901C11.7428 62.5897 15.1317 63.9958 18.6667 64H45.3333C48.8683 63.9958 52.2572 62.5897 54.7568 60.0901C57.2563 57.5905 58.6624 54.2016 58.6667 50.6667V37.3333C60.0812 37.3333 61.4377 36.7714 62.4379 35.7712C63.4381 34.771 64 33.4145 64 32V29.3333C64 26.5044 62.8762 23.7912 60.8758 21.7909C58.8754 19.7905 56.1623 18.6667 53.3333 18.6667ZM32 5.33333C32.7072 5.33333 33.3855 5.61428 33.8856 6.11438C34.3857 6.61448 34.6667 7.29276 34.6667 8C34.3448 10.8342 33.4366 13.5704 32 16.0347C30.5634 13.5704 29.6552 10.8342 29.3333 8C29.3333 7.29276 29.6143 6.61448 30.1144 6.11438C30.6145 5.61428 31.2928 5.33333 32 5.33333ZM5.33333 29.3333C5.33333 27.9188 5.89524 26.5623 6.89543 25.5621C7.89562 24.5619 9.25218 24 10.6667 24H29.3333V32H5.33333V29.3333ZM10.6667 50.6667V37.3333H29.3333V58.6667H18.6667C16.5449 58.6667 14.5101 57.8238 13.0098 56.3235C11.5095 54.8232 10.6667 52.7884 10.6667 50.6667ZM53.3333 50.6667C53.3333 52.7884 52.4905 54.8232 50.9902 56.3235C49.4899 57.8238 47.4551 58.6667 45.3333 58.6667H34.6667V37.3333H53.3333V50.6667ZM34.6667 32V24H53.3333C54.7478 24 56.1044 24.5619 57.1046 25.5621C58.1048 26.5623 58.6667 27.9188 58.6667 29.3333V32H34.6667Z"
                        fill="#5FE26C"
                      />
                    </g>
                    <defs>
                      <clipPath id="clip0_1_195">
                        <rect width="64" height="64" fill="white" />
                      </clipPath>
                    </defs>
                  </svg>
                </div>
                <h5 class="text-[#5FE26C] text-lg font-bold mt-4 mb-4">
                  Gift Cards
                </h5>
                <p class="text-[#90BD95] text-sm font-semibold">
                  Give the gift of exceptional dining with Foodi Gift Cards
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
        </>
    )

    
}
export default home;